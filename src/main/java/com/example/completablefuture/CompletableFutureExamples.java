...
//			completeExceptionallyExample();
	...
	static void completedFutureExample() {
		CompletableFuture<String> cf = CompletableFuture.completedFuture("completed message");
	...
	static void completeExceptionallyExample() {
		...
		CompletableFuture<String> exceptionHandler = cf.handle((s, th) -> "message upon cancel");
	...
	static void allOfExample() {
		...
		CompletableFuture.allOf(futures.toArray(new CompletableFuture[futures.size()]))
				.whenComplete((v, th) -> {
					futures.forEach(cf -> assertTrue(isUpperCase(cf.join())));
					result.append("done");
				});
		assertTrue("Result was empty", result.length() > 0);
	}
